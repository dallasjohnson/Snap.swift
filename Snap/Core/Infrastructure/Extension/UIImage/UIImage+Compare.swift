...
enum CompareError: Error {
    ...
    case notEquals
    case notEqualMetadata([MetaData])

    enum MetaData: String {
        case width, height, bitsPerComponent, bytesPerRow, colorSpace
    }
}
...
extension UIImage {
    ...
    private func compareMetadata(from context1: CGContext, matches context2: CGContext) throws {
        ...
        var imagesHaveEqualMetadata: [CompareError.MetaData] = []
        if !haveEqualWidth { imagesHaveEqualMetadata.append(CompareError.MetaData.width) }
        if !haveEqualHeight { imagesHaveEqualMetadata.append(CompareError.MetaData.height) }
        if !haveEqualBitsPerComponent { imagesHaveEqualMetadata.append(CompareError.MetaData.bitsPerComponent) }
        if !haveEqualBytesPerRow { imagesHaveEqualMetadata.append(CompareError.MetaData.bytesPerRow) }
        if !haveEqualColorSpace { imagesHaveEqualMetadata.append(CompareError.MetaData.colorSpace) }

        guard imagesHaveEqualMetadata.isEmpty else {
            throw CompareError.notEqualMetadata(imagesHaveEqualMetadata)
        }
    }
}
